...
class SettingController extends Controller
{
    ...
    public function saveForumSettings(Request $request)
    {
        $post = $request->all();
        unset($post['_token']);
        foreach ($post as $key => $data) {
    ...
    public function store(Request $request)
    {
        ...
        // return redirect()->back()->with('warning', __('This Action Is Not Allowed Because Of Demo Mode.'));
        if ($request->marketing_banner) {
            $request->validate(
                [
                    'marketing_banner' => 'image|mimes:jpeg,png,jpg,svg|max:3072',
                ]
            );
            $logoName = 'marketing_banner.png';
            $request->file('marketing_banner')->storeAs('uploads/logo/', $logoName);
        }
