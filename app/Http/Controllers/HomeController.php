...
class HomeController extends Controller
{
    ...
    public function index()
    {
        if (!file_exists(storage_path() . "/installed")) {
            ...
            $employee = User::where('type','admin')->count();
            $paid_member = User::where('type','user')->where('payment_status', '1')->count();
            $unpaid_member = User::where('type','user')->where('payment_status','!=', '1')->count();
            $modual = Modual::count();
            $role = Role::count();
            $languages = count(UtilityFacades::languages());

            return view('dashboard.homepage', compact('employee','paid_member', 'unpaid_member', 'modual', 'role', 'languages'));
        }
    }
